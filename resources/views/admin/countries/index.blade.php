                ...
                <div class="panel-body">
                    ...
                    <div class="dataTable_wrapper">
                                ...
                                @isset($countries)
                                    @foreach($countries as $country)
                                        <tr class="odd gradeX text-center">
                                            ...
                                            <td>{{$country->name}}</td>
                                            <?php
                                                if($country->is_activate == 1){$activate = '<span class="badge badge-info">active</span>';}
                                                ...
                                                <ul class="nav navbar-center navbar-top-links" style="border-radius: 15px;">
                                                    <li class="dropdown">
                                                        ...
                                                        <ul class="dropdown-menu dropdown-user">
                                                            <li>
                                                                <a href="{{route('admin/countries/edit', $country->id)}}" style="text-decoration: none; color: white; width: 75px;margin:auto" class="btn btn-success">
                                                                    edit
                                                                </a>
                                                            </li>
                                                            <li class="divider"></li>
                                                            <li >
                                                                <button class="dropdown-item btn btn-danger openDeleteFrom" data-toggle="modal" data-target="#myModalDelete" data-id="{{$country->id}}">
                                                                    delete
                                                                </button>
                                                            </li>
                                                            <li class="divider"></li>
                                                            <li>
                                                                <button class="dropdown-item btn btn-priamry openActivationFrom" data-toggle="modal" data-target="#myModalActivation" data-id="{{$country->id}}">
                                                                    activation
                                                                </button>
                                                            </li>
                                                        </ul>
                        ...
                        <div style="margin-top: 20px; font-weight: 600; font-size: 16px;">
                            Showing 1 to <span id="showItems"></span> of <span>{{App\Models\Country::unArchive()->count()}}</span> entries
                        </div>
...
@section('script')
    ...
    <script>
        ...
        $(document).ready(function() {
            $(document).on('click', '#load_more_button', function() {
                ...
                function load_data(_token) {
                    $.ajax({
                        url: `{{ route("admin/countries/pagination")}}/${offset}/${limit}`,
                        method: "POST",
                        data: {
                            _token: _token,
                        },
                        success: function(data) {
                            if (data.length > 0) {
                                for (let i = 0; i < data.length; i++) {
                                    edit_route =  "{{ route('admin/countries/edit') }}" + '/' + data[i].id;
                                    records += `
                                        <tr class="odd gradeX text-center">
                                            <td>${data[i].id}</td>
                                            <td>${data[i].name}</td>
        ...
        $(document).on('keyup', '.data_search', function() {
            var q = $(this).val();
            var urlPath = "{{ route('admin/countries/search') }}";
