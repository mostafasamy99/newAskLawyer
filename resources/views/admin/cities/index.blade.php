                ...
                <div class="panel-body">
                            ...
                            <div id="dataTables-example_filter" class="dataTables_filter">
                                ...
                                <label>
                                    <div class="form-group">
                                        <select class="form-control select_data_search" name="country_id" id="countries" aria-controls="dataTables-example" style="width: 220px">
                    ...
                    <div class="dataTable_wrapper">
                                ...
                                @isset($cities)
                                    @foreach($cities as $city)
                                        <tr class="odd gradeX text-center">
                                            ...
                                            <?php
                                                if($city->is_activate == 1){$activate = '<span class="badge badge-info">active</span>';}
                                                ...
                                                <ul class="nav navbar-center navbar-top-links" style="border-radius: 15px;">
                                                    <li class="dropdown">
                                                        ...
                                                        <ul class="dropdown-menu dropdown-user">
                                                            <li>
                                                                <a href="{{route('admin/cities/edit', $city->id)}}" style="text-decoration: none; color: white; width: 75px;margin:auto" class="btn btn-success">
                                                                    edit
                                                                </a>
                                                            </li>
                                                            <li class="divider"></li>
                                                            <li >
                                                                <button class="dropdown-item btn btn-danger openDeleteFrom" data-toggle="modal" data-target="#myModalDelete" data-id="{{$city->id}}">
                                                                    delete
                                                                </button>
                                                            </li>
                                                            <li class="divider"></li>
                                                            <li>
                                                                <button class="dropdown-item btn btn-priamry openActivationFrom" data-toggle="modal" data-target="#myModalActivation" data-id="{{$city->id}}">
                                                                    activation
                                                                </button>
                                                            </li>
                                                        </ul>
                        ...
                        <div style="margin-top: 20px; font-weight: 600; font-size: 16px;">
                            Showing 1 to <span id="showItems"></span> of <span>{{App\Models\City::unArchive()->count()}}</span> entries
                        </div>
                        ...
                        <div class="modal-dialog" role="document">
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <form role="form" action="{{url(route('admin/cities/delete'))}}" method="get">
                                        {{ csrf_field() }}
                    ...
                    <div class="modal fade" id="myModalActivation" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
                        ...
                        <div class="modal-dialog" role="document">
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <form role="form" action="{{url(route('admin/cities/activate'))}}" method="get">
                                        {{ csrf_field() }}
                                        <p>Are You Sure To Update This Record ?</p>
...
@section('script')
    ...
    <script>
        ...
        $(document).ready(function() {
            $(document).on('click', '#load_more_button', function() {
                ...
                function load_data(_token) {
                    $.ajax({
                        url: `{{ route("admin/cities/pagination")}}/${offset}/${limit}`,
                        method: "POST",
                        data: {
                            _token: _token,
                        },
                        success: function(data) {
                            if (data.length > 0) {
                                for (let i = 0; i < data.length; i++) {
                                    edit_route =  "{{ route('admin/cities/edit') }}" + '/' + data[i].id;
                                    records += `
                                        <tr class="odd gradeX  text-center">
                                            <td>${data[i].id}</td>
                                            <td>${data[i].name}</td>
        ...
        $(document).on('keyup', '.data_search', function() {
            var q = $(this).val();
            var urlPath = "{{ route('admin/cities/search') }}";
            event.preventDefault();
            search_in_data(_token, q, urlPath)
        });

        $(document).on('change', '.select_data_search', function() {
            var q = $(this).val();
            var record = $(this).attr('name');
            var urlPath = "{{ route('admin/cities/search/byColumn') }}";
            event.preventDefault();
