...
@section('content')
    ...
    <div class="row">
        <div class="col-lg-8">
            {{-- <h1 class="page-header">ProcesSteps</h1> --}}
        </div>
        <div class="col-lg-4">
            ...
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item"><a href="{{route('admin/procesSteps/index')}}/0/{{PAGINATION_COUNT}}">ProcesSteps</a></li>
                    ...
                    <div class="dataTable_wrapper">
                        <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                            ...
                            <tbody id="tableShowData">
                                @isset($procesSteps)
                                    @foreach($procesSteps as $procesStep)
                                        <tr class="odd gradeX text-center">
                                            <td>{{$procesStep->id}}</td>
                                            <td>{{$procesStep->name}}</td>
                                            <!-- <td>
                                                <div class="ml-2 d-flex">
                                                    <img src="{{asset($procesStep->img)}}" alt="admin image"
                                                        class="img-fluid img-50 rounded-circle blur-up lazyloaded" width="100">
                                                </div>
                                            </td> -->
                                            <?php
                                                if($procesStep->is_activate == 1){$activate = '<span class="badge badge-info">active</span>';}
                                                ...
                                                <ul class="nav navbar-center navbar-top-links" style="border-radius: 15px;">
                                                    <li class="dropdown">
                                                        ...
                                                        <ul class="dropdown-menu dropdown-user">
                                                            <li>
                                                                <a href="{{route('admin/procesSteps/edit', $procesStep->id)}}" style="text-decoration: none; color: white; width: 75px;margin:auto" class="btn btn-success">
                                                                    edit
                                                                </a>
                                                            </li>
                                                            <li class="divider"></li>
                                                            <li >
                                                                <button class="dropdown-item btn btn-danger openDeleteFrom" data-toggle="modal" data-target="#myModalDelete" data-id="{{$procesStep->id}}">
                                                                    delete
                                                                </button>
                                                            </li>
                                                            <li class="divider"></li>
                                                            <li>
                                                                <button class="dropdown-item btn btn-priamry openActivationFrom" data-toggle="modal" data-target="#myModalActivation" data-id="{{$procesStep->id}}">
                                                                    activation
                                                                </button>
                                                            </li>
                                                        </ul>
                        ...
                        <div style="margin-top: 20px; font-weight: 600; font-size: 16px;">
                            Showing 1 to <span id="showItems"></span> of <span>{{App\Models\ProcesStep::unArchive()->count()}}</span> entries
                        </div>
                        ...
                        <div class="modal-dialog" role="document">
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <form role="form" action="{{url(route('admin/procesSteps/delete'))}}" method="get">
                                        {{ csrf_field() }}
                    ...
                    <div class="modal fade" id="myModalActivation" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
                        ...
                        <div class="modal-dialog" role="document">
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <form role="form" action="{{url(route('admin/procesSteps/activate'))}}" method="get">
                                        {{ csrf_field() }}
                                        <p>Are You Sure To Update This Record ?</p>
...
@section('script')
    ...
    <script>
        ...
        $(document).ready(function() {
            $(document).on('click', '#load_more_button', function() {
                ...
                function load_data(_token) {
                    $.ajax({
                        url: `{{ route("admin/procesSteps/pagination")}}/${offset}/${limit}`,
                        method: "POST",
                        data: {
                            _token: _token,
                        },
                        success: function(data) {
                            if (data.length > 0) {
                                for (let i = 0; i < data.length; i++) {
                                    ...
                                    edit_route =  "{{ route('admin/procesSteps/edit') }}" + '/' + data[i].id;
                                    records += `
                                        <tr class="odd gradeX text-center">
                                            <td>${data[i].id}</td>
                                            <td>${data[i].name}</td>
        ...
        $(document).on('keyup', '.data_search', function() {
            var q = $(this).val();
            var urlPath = "{{ route('admin/procesSteps/search') }}";
