                ...
                <div class="panel-body">
                    ...
                    <div class="dataTable_wrapper">
                                                ...
                                                <ul class="nav navbar-center navbar-top-links" style="border-radius: 15px;">
                                                    <li class="dropdown">
                                                        ...
                                                        <ul class="dropdown-menu dropdown-user">
                                                            <li >
                                                                <button class="dropdown-item btn btn-danger openBackFrom" data-toggle="modal" data-target="#myModalBack" data-id="{{$lawyer->id}}">
                                                                    back
                                                                </button>
                                                            </li>
                                                        </ul>
                        ...
                        <div style="margin-top: 20px; font-weight: 600; font-size: 16px;">
                            Showing 1 to <span id="showItems"></span> of <span>{{App\Models\Lawyer::archive()->count()}}</span> entries
                        </div>
                        ...
                        <div class="modal-dialog" role="document">
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <form role="form" action="{{url(route('admin/lawyers/back'))}}" method="get">
                                        {{ csrf_field() }}
...
@section('script')
    ...
    <script>
        ...
        $(document).ready(function() {
            $(document).on('click', '#load_more_button', function() {
                ...
                function load_data(_token) {
                    $.ajax({
                        url: `{{ route("admin/lawyers/pagination/archives")}}/${offset}/${limit}`,
                        method: "POST",
                        data: {
                            _token: _token,
                        },
                        success: function(data) {
                            if (data.length > 0) {
                                for (let i = 0; i < data.length; i++) {
                                    ...
                                    edit_route =  "{{ route('admin/lawyers/edit') }}" + '/' + data[i].id;
                                    records += `
                                        <tr class="odd gradeX text-center">
                                            <td>${data[i].id}</td>
                                            <td>${data[i].name}</td>
        ...
        $(document).on('keyup', '.data_search', function() {
            var q = $(this).val();
            var urlPath = "{{ route('admin/lawyers/search/archives') }}";
