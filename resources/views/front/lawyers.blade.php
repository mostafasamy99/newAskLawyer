            ...
            <div class="row">
                <div class="col-md-9 main-content">
                    <div class=" process-content">
                        <h2 class="process-title">المحامين </h2>
                    ...
                    @isset($data['lawyers'])
                            ...
                            @foreach($data['lawyers'] as $lawyer)
                                <div class="col-md-4">
                                    <div class="lawyer-card card">
                                        <img src="{{asset($lawyer['img'])}}" alt="Lawyer Image" class="img-fluid">
                                        <div class="card-body">
                                            <div class="card-meta">
                                                <h5>{{$lawyer['name']}}</h5>
                                            ...
                                            <ul class="row list-feature">
                                                <li class="col">{{$lawyer['title']}} <span><i class="fas fa-user"></i></span></li>
                                                <li class="col">4.5 <span class="rating-star"><i class="fas fa-star"></i></span></li>
                                                <li class="col-12 text-truncate">{{implode(',', array_column($lawyer['languages'], 'name'))}} <span><i class="fas fa-globe-europe"></i></span></li>
                                                <li class="col">{{$lawyer['country']['name']}} ، {{$lawyer['city']['name']}} <span><i class="fas fa-globe-europe"></i></span></li>
                                                <li class="col-12 text-truncate">{{implode(',', array_column($lawyer['services'], 'name'))}} <span><i class="fas fa-bag-shopping"></i></span></li>
                                            </ul>
                                            <a href="{{route('front/lawyer', $lawyer['id'])}}" class="details-btn">التفاصيل</a>
                                        </div>
                                    </div>
                                </div>
                            @endforeach
                        ...
                        <nav aria-label="Page navigation example">
                            <ul class="pagination justify-content-center">
                                ...
                                @php
                                    $lawyers_count = $data['lawyers_count'] / PAGINATION_COUNT_FRONT;
                                @endphp
                                @for ($i = 1; $i <= $lawyers_count; $i++)
                                    <li class="page-item {{ $page == $i ? ' active' : '' }}">
                                        <a class="page-link" href="{{route('front/lawyers', $i)}}">{{ $i }}</a>
                                    </li>
                                @endfor
